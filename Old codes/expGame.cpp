...
int main(){
	...
	for(int i=0;i<9;i++) {
		nn[i]=1;
		for(int j=0;j<i;j++) nn[i]*=i;	
	}
	//for(int i=0;i<9;i++) printf("%d ",nn[i]);
	
	sg[0]=0;
	for(int i=1;i<100005;i++){
		set<int> grundy;
		for(int j=1;i-nn[j]>=0;j++){
			grundy.insert(sg[i-nn[j]]);	
		}
		for(int j=0;;j++) 
			if(grundy.count(j)==0){
				sg[i]=j;
	...
	while(t--){
		...
		if(res==0) puts("Head Chef");
		else puts("Little Chef");
	}
}
